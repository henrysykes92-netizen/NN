...
class Model:
    ...
    def train(self, X, y, *, epochs, batch_size=None, print_every=1, validation_data=None): # epochs=1,
        ...
        for epoch in range(1, epochs + 1):
            ...
            for step in range(train_steps):
                ...
                #print(f'step {step}, traing: {train_steps}')
...
input_size = 15 # rays*2 + (px,py) + (ox,oy) + hunger + state + reward

X = np.random.rand(1000, input_size) # input_size = number of features
y = np.random.randint(0, 3, size=(1000,)) # assuming 3 classes
...
model.add(Layer_Dense(input_size,128, weight_regulariser_l2=5e-4, bias_regulariser_l2=5e-4))
...
model.set(loss=Activation_Softmax_Loss_CategoricalCrossentropy(),
          optimiser=Optimiser_Adam(learning_rate=1e-4, decay=5e-5),
...
model.save(str(f"Data\\Loop_{i}_Model.pkl"))